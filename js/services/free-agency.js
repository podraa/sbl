...
(function () {
  ...
  function normalizePool(pool) {
    ...
    return pool.map(item => {
      if (typeof item === 'string') return { name: SBL.pokemon?.displayName ? SBL.pokemon.displayName(item) : item, points: null };
      return {
        ...item,
        name: SBL.pokemon?.displayName ? SBL.pokemon.displayName(item?.name ?? item?.species ?? '') : String(item?.name ?? item?.species ?? '').trim(),
  ...
  function normalizeRosters(rosters) {
    ...
    for (const [team, list] of Object.entries(rosters || {})) {
      ...
      out[team] = list.map(item => typeof item === 'string'
        ? { name: SBL.pokemon?.displayName ? SBL.pokemon.displayName(item) : item, points: null }
        : { name: SBL.pokemon?.displayName ? SBL.pokemon.displayName(item?.name ?? item?.species ?? '') : String(item?.name ?? item?.species ?? '').trim(), points: item?.points ?? null }
      ).filter(item => item.name);
    }
    return out;
  }
