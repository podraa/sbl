...
(function(){
  ...
  async function cached(key, loader, options={}){
    ...
    // Cache only successful, serialisable read results. If storage quota is
    // exceeded, the live Supabase result is still returned normally.
    try{ await idbPut(key,entry); }catch(_){ }
    return value;
  }
